...
function GameMode:OrderFilter(event)
  --print("[GameMode:OrderFilter] event.units: ")
  --PrintTable(event.units)
  for key, unit in pairs(event.units) do
    --print("[GameMode:OrderFilter] event: ")
    --PrintTable(event)
    if EntIndexToHScript(unit):GetUnitName() == "mortimer" then
      if event.order_type == 1 then --DOTA_UNIT_ORDER_MOVE_TO_POSITION
        --print("[GameMode:OrderFilter] unit: " .. unit)
        EntIndexToHScript(unit):Stop()
        if not EntIndexToHScript(unit).cookie then
          EntIndexToHScript(unit).cookie = true
          Timers:CreateTimer({
            endTime = 0.3, -- when this timer should first execute, you can omit this if you want it to run first on the next frame
            callback = function()
              EntIndexToHScript(unit):SetCursorCastTarget(EntIndexToHScript(unit))
              EntIndexToHScript(unit):GetAbilityByIndex(0):OnSpellStart()
              Timers:CreateTimer({
                endTime = 0.484, -- when this timer should first execute, you can omit this if you want it to run first on the next frame
                callback = function()
                  EntIndexToHScript(unit).cookie = false
                  --turn
                  --cookie
                  --land
                end
    ...
    -- elseif EntIndexToHScript(unit):GetUnitName() == "bomb" then
    --   Timers:CreateTimer({
    --     endTime = 0.06, -- when this timer should first execute, you can omit this if you want it to run first on the next frame
    --     callback = function()
    --       --units' keys are in string
    --       EntIndexToHScript(unit):MoveToPosition(Vector(event.position_x, event.position_y, event.position_z))
    --       --return true
    --     end
    --   })
    end
    ...
    --     endTime = 0.06, -- when this timer should first execute, you can omit this if you want it to run first on the next frame
    --     callback = function()
    --       event.units:MoveToPosition(Vector(event.position_x, event.position_y, event.position_z))
    --       --return true
    --     end
    --   })
