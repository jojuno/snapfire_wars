...
function OnKillEvent(event) {
  ...
  if (teamId !== event.team_id) return;
  ...
  if (event.victory) {
    ...
    pointsToWinPanel.text = $.Localize(
      "#PointsToWin_Victory",
      pointsToWinPanel
    );
    ...
    pointsToWinPanel.text = $.Localize(
      "#PointsToWin_VeryCloseToVictory",
      pointsToWinPanel
    );
...
function UpdateRecentScore() {
  ...
  var curTimeDS = Game.GetGameTime() * 10;
  var recentScore = teamPanel.GetAttributeInt("recent_score_count", 0);
  var timeOfRecentScoreDS = teamPanel.GetAttributeInt(
    "ds_time_of_most_recent_score",
    0
  );
